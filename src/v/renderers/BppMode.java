...
public enum BppMode {
	...
	private static <T, V> SceneRenderer<T, V> SceneGen_8(DoomMain<T, V> DOOM) {
        return (SceneRenderer<T, V>) SceneRendererMode.Parallel.indexedGen.apply((DoomMain<byte[], byte[]>) DOOM);
    }
    
    @SuppressWarnings("unchecked")
    private static <T, V> SceneRenderer<T, V> SceneGen_16(DoomMain<T, V> DOOM) {
        return (SceneRenderer<T, V>) SceneRendererMode.Parallel.hicolorGen.apply((DoomMain<byte[], short[]>) DOOM);
    }
    
    @SuppressWarnings("unchecked")
    private static <T, V> SceneRenderer<T, V> SceneGen_32(DoomMain<T, V> DOOM) {
        return (SceneRenderer<T, V>) SceneRendererMode.Parallel.truecolorGen.apply((DoomMain<byte[], int[]>) DOOM);
    }
